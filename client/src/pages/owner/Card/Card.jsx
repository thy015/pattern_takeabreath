...
function Card() {
  ...
  const handleOggyCard = async () => {
    await axios
      .post(`${BE_PORT}/api/wallet/createWallet?ownerID=${id}`)
      .then((res) => res.data)
    ...
    },
    {
      ...
      render: (_, record) => (
        <Popconfirm
          okText="Xác nhận"
          cancelText="Trở lại"
          onConfirm={() => handleDeleteWoWoCard(record)}
          title="Bạn có chắc gỡ thẻ không ?"
        >
          <Button danger>Gỡ thẻ</Button>
        </Popconfirm>
  ...
  return (
    <div className="h-full ">
      ...
      {isWoWoTransferMoneyPopUp && (
        ...
        >
          ...
          <Form className="h-40">
            ...
            <Button className="float-right" onClick={handleTransferFormSubmit}>
              Transfer
            </Button>
          </Form>
          {/* Add more details as needed */}
      ...
      <div className="flex justify-between items-center w-full my-4">
        ...
        <Button onClick={handleOggyCard} className="mr-4">
          Add New Card
        </Button>
      </div>
      <Table
        columns={wowoColumns}
        dataSource={wowoCardsData}
        scroll={{ x: "max-content" }}
      ></Table>
    </div>
  );
