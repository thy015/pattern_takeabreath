...
function FormRoom({ isVisible, close }) {
  ...
  useEffect(() => {
    axios
      .get(`${BE_PORT}/api/hotelList/roomTypes`)
      .then((res) => res.data)
      .then((data) => {
    ...
    axios
      .get(`${BE_PORT}/api/hotelList/hotelOwner`)
      .then((res) => res.data)
      .then((data) => {
  ...
  const onFinish = async (values) => {
    ...
    if (isEmpty(selectedRoom)) {
      axios
        .post(`${BE_PORT}/api/hotelList/createRoom`, formInput)
        .then((res) => res.data)
        .then((data) => {
          const room = {
            ...data.data,
  ...
  return (
    <>
      ...
      >
        <div className="text-[20px] font-bold items-center text-center  text-blue-900 m-[10px] mb-[20px]">
          {isEmpty(selectedRoom) ? "Thêm phòng" : "Cập nhật phòng"}
        </div>
        <Form
          onFinish={onFinish}
          form={form}
        ...
        >
          ...
          <Form.Item
            ...
            name={"capacity"}
            rules={[
              { required: true, message: "Vui lòng nhâp không gian phòng !" },
            ]}
          >
            <InputNumber
              className="w-[100%]"
          ...
          <Form.Item
            ...
            name={"numberOfRooms"}
            rules={[
              { required: true, message: "Vui lòng nhâp số lượng phòng !" },
            ]}
          >
            <InputNumber
              className="inputnumber-room w-[30%]"
              max={100}
              min={1}
          ...
          <Form.Item
            ...
            name={"numberOfBeds"}
            rules={[
              { required: true, message: "Vui lòng nhâp số lượng giường !" },
            ]}
          >
            <InputNumber
              max={30}
