...
function FormCard({ visible, close }) {
  ...
  const onFinish = (value) => {
    ...
    if (numberCard.length !== 16) {
      openNotification("error", "Số thẻ phải đủ 16 số", "");
      return;
    }
    if (expDay.isBefore(currentDay)) {
      openNotification("error", "Ngày hết hạn phải ở tương lai", "");
      return;
    }

    // thời hạn thẻ phải ít nhất 1 năm
    if (!expDay.isAfter(currentDay.add(1, "year"))) {
      openNotification("error", "Ngày hết hạn ít nhất là 1 năm sau", "");
      return;
    }
