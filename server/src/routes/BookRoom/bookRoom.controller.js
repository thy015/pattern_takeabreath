...
const bookRoom = async (req, res) => {
  ...
  try {
    ...
    const hotel = await HotelRoomFacade.getHotelById(idHotel);
    const room = await HotelRoomFacade.getRoomById(idRoom);
...
const completedTran = async (req, res) => {
  ...
  try {
    ...
    const roomMatch = await HotelRoomFacade.getRoomById(invoice.roomID);
    const hotelMatch = await HotelRoomFacade.getHotelById(invoice.hotelID);
    ...
    if (paidRoomsInvoice.length > 0) {
      let bookingInfo = await Promise.all(
          paidRoomsInvoice.map(async (invoice) => {
            let roomInfo = await HotelRoomFacade.getRoomById(invoice.roomID);
            let hotelInfo = await HotelRoomFacade.getHotelById(invoice.hotelID);
